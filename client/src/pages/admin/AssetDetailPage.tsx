...
export function AssetDetailPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Tab bar */}
      <div className="flex z-20 gap-0 border-b border-af-border">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            ...
            <Card>
              ...
              <dl className="grid grid-cols-2 gap-x-6 gap-y-4">
                {[
                  ...
                  {
                    label: "Assigned to",
                    value: asset.assignedUser?.name ?? "Unassigned",
                  },
                  {
                    label: "Purchase date",
                  ...
                  <div key={row.label}>
                    <dt className="font-mono text-[10px] uppercase tracking-[0.1em] text-af-muted mb-0.5">
                      {row.label}
                    </dt>
                    <dd
                      className={cn("text-[13px]", row.mono ? "font-mono" : "")}
                    >
                      {row.value}
                    </dd>
                  </div>
            ...
            {asset.lastLat != null && asset.lastLong != null && (
              <div className="rounded-lg border border-af-border bg-white p-4">
                ...
                <p className="font-mono text-[12px]">
                  {asset.lastLat.toFixed(5)}, {asset.lastLong.toFixed(5)}
                </p>
                {asset.lastLocationAt && (
                  <p className="text-[11px] text-af-muted mt-0.5">
                    {formatRelative(asset.lastLocationAt)}
                  </p>
                )}
              </div>
            )}
              ...
              {(schedules ?? []).length === 0 ? (
                ...
                (schedules ?? []).map((s) => (
                  <Tr key={s.id} onClick={() => navigate("/maintenance")}>
                    ...
                    <Td mono>
                      {s.scheduleType === "time_based" ? "Time" : "Usage"}
                    </Td>
                    <Td>
                    ...
                    <Td>
                      <span
                        className={cn(
                          ...
                          s.active
                            ? "bg-af-ok-100 text-af-ok-600"
                            : "bg-af-ink-100 text-af-muted",
                        )}
                      >
                        {s.active ? "Yes" : "No"}
                      </span>
                ...
                <div key={h.id} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px]">
                      <span className="font-medium">{h.changedBy}</span> changed{" "}
                      <span className="font-mono text-[11px] text-af-muted">
                        {h.field}
                      </span>{" "}
                      from{" "}
                      <span className="font-mono text-[11px] bg-af-crit-100 text-af-crit-600 px-1 rounded">
                        {h.oldValue || "—"}
                      </span>{" "}
                      to{" "}
                      <span className="font-mono text-[11px] bg-af-ok-100 text-af-ok-600 px-1 rounded">
                        {h.newValue}
                      </span>
                    </p>
                  </div>
                  <p className="font-mono text-[10px] text-af-muted flex-shrink-0">
